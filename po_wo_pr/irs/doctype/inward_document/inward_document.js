...
//         // 1. Get current logged-in employee's prefix
//         const res = await frappe.db.get_value(
//             "Employee",
//             { user_id: frappe.session.user },
//             "custom_ma_code_prefix"
//         );

//         if (res && res.message && res.message.custom_ma_code_prefix) {
//             const user_prefix = res.message.custom_ma_code_prefix;

//             // 2. Filter the 'maa_series' field choices to match user's prefix
//             frm.set_query("maa_series", function () {
//                 return {
//                     filters: {
//                         "name": user_prefix  // Filters the record name directly (e.g., MFBH)
//                     }
//                 };
//             });

//             // 3. Auto-set the value for new records
//             if (frm.is_new() && !frm.doc.maa_series) {
//                 frm.set_value("maa_series", user_prefix);
//             }
//         }
//     },
...
frappe.ui.form.on("Inward Document", {
    ...
    refresh(frm) {
        ...
        // Remove old dynamic buttons on refresh
        frm.remove_custom_button("Add Student Entry");
        frm.remove_custom_button("View Student");

        // 1. If student_entry or maa_code is populated, show View Student
        const linked_student = frm.doc.student_entry || frm.doc.maa_code;
        if (linked_student) {
            frm.add_custom_button("View Student", () => {
                frappe.set_route("Form", "Student", linked_student);
            });
            return;
        }
