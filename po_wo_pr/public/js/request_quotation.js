frappe.ui.form.on("Request for Quotation", {
    ...
    onload(frm) {
        ...
        frm.set_query("email_template", () => {
            return {
                filters: [
                    ["Email Template", "name", "like", "%Request for Quotation%"]
                ]
            };
        });

        if (frm.is_new()) {
            frm._last_fetched_mr = null;
...
// Pulls the contact numbers text from the Material Request the first item row
// came from. Runs once per Material Request -- the guard keeps repeated
// items_add events from re-fetching the same values.
function fetch_contact_details(frm) {
    if (!frm.doc.items || !frm.doc.items.length) return;
    const mr_name = frm.doc.items[0].material_request;
    if (!mr_name) return;
    if (frm._last_fetched_mr === mr_name) return;
    frm._last_fetched_mr = mr_name;
    frappe.db
        .get_value("Material Request", mr_name, "custom_contact_numbers")
        .then((r) => {
            const numbers = r.message && r.message.custom_contact_numbers;
            if (numbers) {
                frm.set_value("custom_contact_numbers", numbers);
            }
        });
}
